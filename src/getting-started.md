...
флэш-карточек (англ. _flashcard_) [прим. перев. Ещё встречается перевод как
...
_default_); все карточки, которые каким-то образом отделились от других колод,
...
предоставить ей шаблон (англ. _blueprint_) в котором указано, какие поля должны
...
При создании новой коллекции [прим. перев. _профиля_], Anki автоматически
...
введён какой-либо текст [прим. перев. _Любой текст, чтобы эта функция
сработала_], обратная карточка будет создана. Дополнительные сведения об этом
см. в разделе [Шаблоны карточек](templates/intro.md)**Основная (с обратной карточкой)**  
...
**Основная (введите ответ)** 
По сути, это Основная, с дополнительным текстовым полем на лицевой стороне, где
вы можете ввести свой ответ, после переворачивания на оборот, введённый вами
текст будет проверен и сравнён с ответом. Дополнительные сведения об этом
см. в разделе [Проверка своего ответа](templates/fields.md#Проверка-своего-ответа).

**Задание с пропусками**  
Тип записи который облегчает выбор текста и превращение его в обучающие пропуски
(англ. _cloze deletion_) (например, "Человек приземлился на Луну в \[…​\] году"
...
или практика в реальных условиях (англ. _exposure to real-world sentences_).
...
или фактов из викторин в пабе (англ. _pub quiz trivia_), вам, вероятно, не нужен
